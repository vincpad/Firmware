...
void BlockLocalPositionEstimator::publishOdom()
{
	...
	if (PX4_ISFINITE(_x(X_x)) && PX4_ISFINITE(_x(X_y)) && PX4_ISFINITE(_x(X_z)) &&
		...
		// angular velocity
		_pub_odom.get().rollspeed = _sub_angular_velocity.get().rollspeed;	// roll rate
		_pub_odom.get().pitchspeed = _sub_angular_velocity.get().pitchspeed;	// pitch rate
		_pub_odom.get().yawspeed = _sub_angular_velocity.get().yawspeed;	// yaw rate
